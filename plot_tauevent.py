...
def plotEvent(img, mins, maxs, str_):
    ...
    if maxs[-3] > 0 : plt.imshow(img[-1,4,:,:], cmap='Blues', norm=LogNorm(), alpha=0.9, vmin=mins[-3], vmax=maxs[-3])
    if maxs[-4] > 0 : plt.imshow(img[-1,3,:,:], cmap='Greens', norm=LogNorm(), alpha=0.9, vmin=mins[-4], vmax=maxs[-4])
