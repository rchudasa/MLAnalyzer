...
class ParquetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data['X_CMSII'] = np.float32(data['X_CMSII'])
        ...
        data['X_CMSII'][-1,...] = 25.*data['X_CMSII'][-1,...] # For HCAL: to match pixel intensity dist of other layers
...
def plotJet(img, mins, maxs, str_):
    ...
    if maxs[-2] > 0 : plt.imshow(img[-1,5,:,:], cmap='Greens', norm=LogNorm(), alpha=0.9, vmin=mins[-2], vmax=maxs[-2])
    if maxs[-3] > 0 : plt.imshow(img[-1,4,:,:], cmap='Purples', norm=LogNorm(), alpha=0.9, vmin=mins[-3], vmax=maxs[-3])
...
for i, data in enumerate(train_loader):
    ...
    plotJet(img, mins, maxs, 'images/tau_event%d.png'%(i))
