...
struct tau1_tau2_map {
  std::vector<unsigned int> recoTau1Idxs;
  std::vector<unsigned int> recoTau2Idxs;
};
std::vector<tau1_tau2_map> vDiTaus;
...
bool RecHitAnalyzer::runEvtSel_jet_dijet_ditau( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  ...
  if ( debug ) std::cout << " Tau collection size:" << taus->size() << std::endl;
    ...
    std::vector<unsigned int> vKinTauIdxs;  
    ...
    for ( unsigned iT1(0); iT1 != taus->size(); ++iT1 ) {
      ...
      if ( debug) std::cout << " Tau pt :" << iTau1->pt() << "  " << iTau1->eta() << " Tau MVA rejection score:" << (*MVAIsolation)[iTau1] << std::endl;
      vKinTauIdxs.push_back( iT1 );
      
    }
    if ( vKinTauIdxs.size() < 2 ) {
      hNpassed_kin->Fill(0);
      return false;
    }
    nKinTau_ = vKinTauIdxs.size();
    
    std::cout << "number of taus after pt cut:" << vKinTauIdxs.size() << std::endl;
    hNpassed_kin->Fill(1.);
    
    // Ensure two taus passing basic MVA identification cuts such as electron & muon rejection
    hNpassed_MVA->Fill(0);
    std::vector<unsigned int> vTaus;  
    
    for ( unsigned int iP : vKinTauIdxs ) {
      reco::PFTauRef iTau1( taus, iP );
    ...
    if ( debug ) std::cout << " Taus passing MVA cut:" << vTaus.size() << std::endl;
    if ( vTaus.size() < 2 ) {
      hNpassed_MVA->Fill(0);
      return false;
    }
    ...
    std::vector<unsigned int> tau1Idx;
    std::vector<unsigned int> tau2Idx;
    unsigned int tau_combinations = 0;
    
    // make various tau combination
    for ( unsigned int j = 0; j < vTaus.size()-1; j++ ) {
      reco::PFTauRef iTau1( taus, vTaus[j] );
      
      for ( unsigned int k = j+1; k < vTaus.size(); k++ ) {
	reco::PFTauRef iTau2( taus, vTaus[k] );
    ...
    if ( debug) std::cout << "There is " << tau_combinations  << " tau combinations" << std::endl; 
    if(tau1Idx.empty() || tau2Idx.empty()) return false;
    
    
    tau1_tau2_map ditau_obj = {tau1Idx,tau2Idx};
    vDiTaus.push_back(ditau_obj);
    
    std::cout << " vDiTau size:" << tau1Idx.size() << " "<< tau2Idx.size() << std::endl;
    ...
    // measure Ditau mass using fast MTT method
    for ( unsigned int j = 0; j < tau1Idx.size(); j++ ) {
      reco::PFTauRef iTau1( taus, tau1Idx[j] );
      reco::PFTauRef iTau2( taus, tau2Idx[j] );
      std::cout<< "In combination loop"<< iTau1->pt() << "  " << iTau1->eta() << "  tau2: " << iTau2->pt() << "  " << iTau2->eta() << std::endl;
	...
	if ( debug ) std::cout << " Tau pair " << " ("<< iTau1->pt() << " GeV) + " << " (" << iTau2->pt() << " GeV) | ditau Mvis : " << diMvis << " GeV | Tau dR = " << taudR << " jet dR: " << jetdR <<  std::endl; 
